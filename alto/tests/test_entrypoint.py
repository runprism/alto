...
def test_bad_types():
    ...
    with pytest.raises(ValueError) as cm:
        _ = BaseEntrypoint(entrypoint_conf=conf, alto_wkdir=ENTRYPOINT_WKDIR)
...
def test_function_bad_cmd():
    ...
    with pytest.raises(ValueError) as cm:
        _ = FunctionEntrypoint(entrypoint_conf=conf, alto_wkdir=ENTRYPOINT_WKDIR)
...
def test_jupyter_normal_entrypoint():
    ...
    assert entrypoint.cmd == "alto_nb.ipynb"
    assert entrypoint.kernel == "python3"
    assert entrypoint.notebook_path == "alto_nb.ipynb"
    assert entrypoint.output_path == "alto_nb_exec.ipynb"
...
def test_jupyter_bad_command():
    ...
    with pytest.raises(ValueError) as cm:
        _ = JupyterEntrypoint(entrypoint_conf=conf, alto_wkdir=ENTRYPOINT_WKDIR)
